...
If a process with a lower burst time arrives (or in waiting queue), the current one is interputed.
If a process with a same burst time arrives (or in waiting queue), the current one is prefered.
